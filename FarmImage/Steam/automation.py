...
if __name__ == '__main__':
    ...
    STEAM_FLAG = os.getenv('STEAM_FLAG', 'True')
    YT_FLAG = os.getenv('YT_FLAG', 'True')
    STEAM_RANGE = os.getenv('STEAM_RANGE', '4')
    ...
    while True:
        ...
        # if STEAM_FLAG == "True" or  STEAM_FLAG == True:
        #     # 3 Steam cycles
        #     for i in range(1, int(STEAM_RANGE)):
        #         ok = steam_cycle(i, INSTALL_DIR, STEAMCMD_PATH, USERNAME, PASSWORD, APP_ID)
                ...
                # time.sleep(INTERVAL)
        if YT_FLAG == "True" or  YT_FLAG == True:
            # 3 yt-dlp cycles
